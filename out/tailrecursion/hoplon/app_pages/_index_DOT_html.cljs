...
(defc= deal-label (case game-state :first-deal "deal" :second-deal "deal" :game-over "try again"))
...
(defn hold-state "Set the hold state on a card" [idx state] (swap! current-hand (fn [value] (assoc-in (vec value) [idx 1 :held] state))))

(defn apply-to-hand "Run the given function against all cards in the hand\n  \n  Func takes three arguments:\n     - the index of the card\n     - the card code\n     - info about the card (map of url, held state)\n  " [func] (doseq [[idx [card info]] (clojure.core/deref current-hand-index)] (func idx card info)))

(defn toggle-hold "Hold/Unhold a card, but only when the user is allowed to\n   do so (they get two chances)" [idx] (if (not (clojure.core/deref lock-holds)) (swap! current-hand (fn [value] (update-in (vec value) [idx 1 :held] not)))))
...
(html (head (title "Hoplon Video Poker") (link :rel "stylesheet" :href "css/style.css" :type "text/css")) (body (h1 "Video Poker!") (div :text total) (div :class "current-hand" :text what-do-i-have) (div (button :click new-cards :text (cell= deal-label))) (div :class "hand" (loop-tpl :bindings [[idx [card info]] current-hand-index] (div :class "card" (div :class "indicator" :do-class (cell= {:part-of-hand (boolean (some #{card} scored-cards))}) (img :src (cell= (:url info)) :do-class (cell= {:selected (get-in current-hand [idx 1 :held])}) :click (fn* [] (toggle-hold (clojure.core/deref idx))))))))))
